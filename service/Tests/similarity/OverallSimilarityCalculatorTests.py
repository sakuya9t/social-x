...
class OverallSimilarityCalculatorTests(unittest.TestCase):
    def test_calc_realtime(self):
        score = OverallSimilarityCalculator()._calc(data1, REALTIME_MODE)
        print(score)
        self.assertTrue(0 <= score <= 1)

    def test_calc_batch(self):
        score = OverallSimilarityCalculator()._calc(data2, BATCH_MODE)
        print(score)
        self.assertTrue(0 <= score <= 1)

    def test_calc_autodetect_type(self):
        score1 = OverallSimilarityCalculator()._calc(data1, REALTIME_MODE)
        score2 = OverallSimilarityCalculator().calc(data1)
        self.assertEqual(score1, score2)
        score1 = OverallSimilarityCalculator()._calc(data2, BATCH_MODE)
        score2 = OverallSimilarityCalculator().calc(data2)
        self.assertEqual(score1, score2)
